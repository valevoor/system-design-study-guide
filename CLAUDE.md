...
6. **The AI-authorship disclosure and the Draft warning live once, in `README.md`
   only.** The former says the guide was created by AI and steered by a human; the
   latter says the guide is a work in progress not yet fully reviewed for accuracy.
   Don't delete either and don't propagate them into individual chapter files — same
   reasoning as the no-footer rule in §4: one honest, visible statement beats fifteen
   repeated ones. Remove the Draft warning only when the user says the guide is ready
   (e.g. all chapters have been reviewed) — don't remove it unilaterally.
...
- ❌ Don't remove the AI-authorship disclosure or the Draft warning from `README.md`,
  and don't copy either into individual chapter files (see golden rule 6 in §2).
